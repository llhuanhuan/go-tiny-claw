...
// StreamGenerate 实现了 LLMProvider 接口的流式方法，直接委托给底层 Provider。
// 注意：流式模式下的计费追踪需要消费 channel 中的 Usage 事件，这里暂不包装，
// 实际的计费通过 Generate (阻塞式) 路径完成，引擎的主循环使用的是 Generate。
func (t *CostTracker) StreamGenerate(ctx context.Context, msgs []schema.Message, availableTools []schema.ToolDefinition) (<-chan provider.StreamEvent, error) {
	return t.nextProvider.StreamGenerate(ctx, msgs, availableTools)
}
